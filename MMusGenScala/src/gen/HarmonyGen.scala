...
case class HarmonyGen(melody: MusicalSegment) {
          ...
          (all2 map { y =>
            {
              if (y._1._1._1.length < 2 && y._2) y._1._1._1.flatten
                ...
                findChordsC(y._1._1._1, y._1._1._2, y._1._2) match {
                  case Some(possC) => possC
                  case None => {
                    println("could not solve one part with constraints, try with the linear harmonizer for possible diagnostic,\n but not all composer constraints will be satisfied.")
  ...
  def prevPoss(ci: HavePrev): List[ChInv] = {
    ci match {
      ...
      case ChInv(Triad(I(_, None)), Inv2) => ChInv(Triad(VI), Fond) :: HarmFct(IV)
      case ChInv(Triad(II(_, None)), _) => HarmFct(I)
      case ChInv(Triad(III(_, None)), _) => getCiL(List(I), List(Fond, Inv1))
      case ChInv(Triad(IV(_, None)), _) => HarmFct(I)
      case ChInv(Triad(V(_, None)), _) => ChInv(Triad(I), Inv2) :: HarmFct(I) ::: HarmFct(IV)
      case ChInv(Triad(VI(_, None)), _) => ChInv(Triad(III), Fond) :: getCiL(List(V), List(Fond, Inv1))
